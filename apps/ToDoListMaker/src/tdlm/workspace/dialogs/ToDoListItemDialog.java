...
public class ToDoListItemDialog extends Stage {
    ...
    private void processCompleteWork() {
        ...
        if (editing) {
            if (data.isItemSelected() && (data.isValidNewToDoItem(category, description, startDate, endDate, assignedTo, completed))) {
                ...
                // @todoItemsController
                AppDialogsFacade.showMessageDialog(app.getGUIModule().getWindow(), TDLM_ITEM_INVALID_TITLE, TDLM_ITEM_INVALID_MESSAGE);
            }
        }
        // IF WE ARE ADDING
        else {
